    ...
    Profile.findOne({ user: req.user.id })
      ...
      .then(profile => {
        if (!profile) {
          ...
          return res.status(404).json({ msg: "error", errors, isValid: false });
        }
        res.json(profile);
      })
      .catch(err => {
...
router.post(
  ...
  (req, res) => {
    ...
    if (typeof req.body.skulls != "undefined") {
...
// @route   POST api/pofile/experience
...
// @route   POST api/pofile/education
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        ...
        if (removeIndex < 0) {
          errors.noexperience = "That experience don't exists";
          return res.status(404).json({ msg: "error", errors, isValid: false });
        }
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id })
      .then(profile => {
        ...
        if (removeIndex < 0) {
          errors.noeducation = "That education don't exists";
          return res.status(404).json({ msg: "error", errors, isValid: false });
        }
